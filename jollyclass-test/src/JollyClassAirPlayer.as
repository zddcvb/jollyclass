package
{
	...
	public class JollyClassAirPlayer extends Sprite
	{
		...
		private var bunlk_loader:BulkLoader;
		...
		private function loaderSwf(swfPath:String):void
		{
			var _context:LoaderContext=new LoaderContext();
			_context.allowCodeImport=true;
			_context.applicationDomain=ApplicationDomain.currentDomain;
			bunlk_loader=new BulkLoader("main swf");	
			bunlk_loader.add(swfPath,{id:"swf",context:_context});
			bunlk_loader.addEventListener(BulkLoader.COMPLETE,onBulkComplete);
			bunlk_loader.addEventListener(BulkLoader.PROGRESS,onBulkProgress);
			bunlk_loader.start();
		}
		
		protected function onBulkProgress(event:Event):void
		{
			
		}
		
		protected function onBulkComplete(event:Event):void
		{
			_mc=bunlk_loader.getMovieClip("swf");
			AneUtils.showShortToast(_mc.name);
			addChild(_mc.parent);
